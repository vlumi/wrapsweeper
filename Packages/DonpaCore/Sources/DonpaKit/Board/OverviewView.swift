...
/// Fullscreen board overview for navigating a board too big to see at once: the
/// whole board as a downsampled image with a "you are here" rectangle; drag or tap
/// to recentre the live board there. Dismissed by the X or a backdrop tap. (The
/// corner minimap is just a position indicator + the entry point to this view.)
struct OverviewView: View {
    ...
    /// Drives the image refresh when the board changes.
    let revision: Int

    /// Live viewport rect (normalized, y-down) — re-read each drag so the box
    /// tracks where the (clamped) camera landed.
    ...
    var body: some View {
        ZStack {
            ...
            GeometryReader { geo in
                // Fit the board aspect into the available space.
                let board = CGSize(
                ...
                overviewImage
                    ...
                    // Gesture bound to the IMAGE only, so taps outside it fall
                    // through to the backdrop and close the overview.
                    .contentShape(Rectangle())
                    .gesture(
        ...
        #if os(macOS)
        // Hold keyboard focus (ring suppressed) so Esc closes the overview, which
        // the SpriteKit board would otherwise swallow. `.onExitCommand` is the
        // proper Escape hook; macOS-only (unavailable on iOS).
        .focusable()
        .focused($focused)
        .focusEffectDisabled()
    ...
    /// Pixels-per-cell for the fullscreen image — capped for a sane bitmap, denser
    /// than the corner minimap since it's shown big.
    ...
    /// Map an image-space point to a normalized board point, drive the camera there,
    /// and re-read the viewport so the box tracks where the camera landed.
