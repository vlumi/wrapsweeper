...
Both targets (`Donpa-iOS`, `Donpa-macOS`) carry their own copy of these — keep
them in step.
...
4. **Tag the commit** with a SemVer tag, suffix for pre-releases:

   ```
   v0.1.0-beta.1     # TestFlight beta (dot before the number — sorts correctly)
   v0.1.0-rc.1       # release candidate
   v0.1.0            # store release (no suffix)
   ```

   ```sh
   git tag -a v0.1.0-beta.2 -m "Donpa Squad 0.1.0 beta 2"
   git push origin v0.1.0-beta.2
   ```

5. **GitHub Release** from the tag — mark betas/RCs as **pre-release** so they
   don't show as "Latest". Record the Apple build number and commit in the body:

   ```sh
   gh release create v0.1.0-beta.2 --title "v0.1.0-beta.2 — Donpa Squad" \
...
and distributed independently; the metadata is shared but entered per-record,
and screenshots are per-device-size.
